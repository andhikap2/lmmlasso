...
pd_snps=pd_snps.iloc[:,-len(pd_snps.columns)+1:-1]
pd_snps=pd_snps.iloc[:,np.random.permutation(np.arange(len(pd_snps.columns)))]
...
pd_microclim=pd_microclim.iloc[:,np.random.permutation(np.arange(len(pd_microclim.columns)))] 
...
X=X.iloc[:,np.random.permutation(np.arange(len(X.columns)))]
...
plt.title('Lasso shuffled alpha=0.001')
plt.xlabel('DTB_observed')
plt.ylabel('DTB_predicted')
plt.plot(HalleFall2006.iloc[:,1],HalleFall2006.iloc[:,2],'ro',label='HalleFall2006')
plt.plot(NorwichSummer2006.iloc[:,1],NorwichSummer2006.ilov[:,2],'gX',label='NorwichSummer2006')
...
plt.plot(NorwichFall2006.iloc[:,1],NorwichFall2006.iloc[:,2],'rd',label='NorwichFall2006')
plt.plot(OuluFall2007.iloc[:,1],OuluFall2007.iloc[:,2],'rP',label='OuluFall2007')
