...
class PredMap(Lasso):
	...
	# def getIBSmatrix(self): #unsure if this works
	# 	"""
	# 	calculate the ibs matrix *this takes a very long time*
	# 	"""
	# 	G=self.G
	# 	(n,p)=G.shape
	# 	K=np.zeros((n,n))
	# 	for i in range(n):
	# 		for j in range(n):			
	# 			total=sum(operator.eq(G[i,:],G[j,:]))
	# 			prop=operator.truediv(total,p)
	# 			K[i,j]=prop
	# 			K[j,i]=prop
	# 	self.K=K
	# 	return self.K
