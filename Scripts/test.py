#Mixed model lasso incorporating random effects (relationship matrix)

import numpy as np
from sklearn.linear_model import Lasso
from sklearn.metrics import mean_squared_error
from sklearn.model_selection import cross_validate
from limix.vardec import VarDec as var ## This only exists in version 3.0.x
...
def runCrossValidation(estimator,X,y,alphas,K=None,n_folds=10,verbose=False):
	...
	""" setting up """
	N = X.shape[0]
	kf = KFold(n_splits=n_folds,shuffle=True) #Split the data into K-folds and shuffle the order	
	n_alphas = len(alphas)
...
class LmmLasso(Lasso):
	...
	def fit(self, X, y, K, standardize=False, verbose=False,**lasso_args):
	    ...
	    if standardize:
	        ...
	        y / y.std(axis=0)
	    """ training null model """
	    vd = var(y) #The variance decomposition of y
	    vd.addRandomEffect(is_noise=True)
